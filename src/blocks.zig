...
pub const meshes = struct { // MARK: meshes
	...
	fn readTextureData(_path: []const u8) void {
		...
		const textureInfoPath = extendedPath(path, &buffer, "_textureInfo.zon");
		const textureInfoZon = main.files.readToZon(main.stackAllocator, textureInfoPath) catch .ZonNull;
