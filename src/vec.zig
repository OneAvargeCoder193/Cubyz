...
pub const Mat4f = struct { // MARK: Mat4f
	...
	pub fn orthographic(left: f32, right: f32, bottom: f32, top: f32, near: f32, far: f32) Mat4f { // zig fmt: off
		return Mat4f{
			.rows = [4]Vec4f{
				Vec4f{2.0 / (right - left), 0, 0, -(right + left) / (right - left)},
				Vec4f{0, 0, 2.0 / (top - bottom), -(top + bottom) / (top - bottom)},
				Vec4f{0, (far + near)/(near - far), 0, -2*near*far/(near - far)},
				Vec4f{0, 0, 0, 1},
			},
		};
	...
	pub fn orthographicInverse(left: f32, right: f32, bottom: f32, top: f32, near: f32, far: f32) Mat4f { // zig fmt: off
		return Mat4f{
			.rows = [4]Vec4f{
				Vec4f{(right - left)/2.0, 0, 0, (right + left)/2.0},
				Vec4f{0, 0, (top - bottom)/2.0, (top + bottom)/2.0},
				Vec4f{0, (near - far)/(far + near), 0, 2*near*far/(far + near)},
				Vec4f{0, 0, 0, 1}
			},
		};
	} // zig fmt: on
