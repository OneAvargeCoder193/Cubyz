// Thanks to https://github.com/zig-wasm/wasmer-zig-api for the wrappers
...
pub const InstanceEnv = struct {
    instance: *Instance,
    memory: *Memory,

    pub fn alloc(self: *InstanceEnv, module: *Module, size: u32) ![*]u8 {
        const func = self.instance.getExportFunc(module, "alloc") orelse
            return error.MissingAlloc;
        defer func.deinit();

        const ptr: u32 = try func.call(u32, .{size});
        return self.memory.data()[ptr..];
    }

    pub fn free(self: *InstanceEnv, module: *Module, ptr: [*]u8) !void {
        const func = self.instance.getExportFunc(module, "free") orelse
            return error.MissingFree;
        defer func.deinit();

        const offset: u32 = @intCast(ptr - self.memory.data());
        _ = try func.call(void, .{offset});
    }

    pub fn dupe(self: *InstanceEnv, module: *Module, data: []const u8) ![*]u8 {
        const dest = try self.alloc(module, @intCast(data.len));

        std.mem.copyForwards(u8, dest[0..data.len], data);
...
pub const Func = opaque {
    ...
    pub fn init(store: *Store, callback: anytype, env: *InstanceEnv) !*Func {
        ...
        // params
        var args = ValtypeVec.initWithCapacity(fn_info.params.len - 1);
        inline for (fn_info.params[1..], 0..) |p, i| {
            args.data[i] = Valtype.init(typeToValKind(p.type.?));
        }
        std.debug.print("{d}\n", .{args.size});

        // results
        const has_return = fn_info.return_type.? != void;

        var results = ValtypeVec.initWithCapacity(if(has_return) 1 else 0);
        if (has_return) {
            results.data[0] = Valtype.init(typeToValKind(fn_info.return_type.?));
        }

        const functype = wasm_functype_new(&args, &results) orelse return Error.FuncInit;
        defer wasm_functype_delete(functype);

        return wasm_func_new_with_env(store, functype, makeCallback(callback), @ptrCast(env), null) orelse Error.FuncInit;
    }

    fn makeCallback(func: anytype) Callback {
        const F = @TypeOf(func);
        const fn_info = @typeInfo(F).@"fn";

        return struct {
            fn cb(env: *anyopaque, params: *const ValVec, results: *ValVec) callconv(.c) ?*Trap {
                const instanceEnv: *InstanceEnv = @ptrCast(@alignCast(env));
                const memory = instanceEnv.memory;

                var args: blk: {
                    var argument_field_list: [fn_info.params.len - 1]type = undefined;
                    for (fn_info.params[1..], 0..) |arg, i| {
                        const T = arg.type orelse @compileError("cannot create ArgsTuple for function with an 'anytype' parameter");
                        argument_field_list[i] = T;
                    }
                    break :blk @Tuple(&argument_field_list);
                } = undefined;

                inline for (fn_info.params[1..], 0..) |p, i| {
                    args[i] = fromValue(p.type.?, memory, params.data[i]);
                }

                const ret = @call(.auto, func, .{instanceEnv} ++ args);
                if (fn_info.return_type.? == void) return null;

                const R = fn_info.return_type.?;
                if (@typeInfo(R) == .@"struct" and @typeInfo(R).@"struct".is_tuple) {
                    inline for (@typeInfo(R).@"struct".fields, 0..) |field, i| {
                        results.data[i] = toValue(@field(ret, field.name), memory);
                    }
                } else {
                    results.data[0] = toValue(ret, memory);
                }
    ...
    fn fromValue(comptime T: type, memory: *Memory, v: Value) T {
        return switch (T) {
            i32 => v.of.i32,
            u32 => @bitCast(v.of.i32),
            i64 => v.of.i64,
            u64 => @bitCast(v.of.i64),
            [*]u8 => memory.data()[@intCast(v.of.i32)..],
            f32 => v.of.f32,
            f64 => v.of.f64,
    ...
    fn toValue(v: anytype, memory: *Memory) Value {
        return switch (@TypeOf(v)) {
            i32 => .{ .kind = .i32, .of = .{ .i32 = v } },
            u32 => .{ .kind = .i32, .of = .{ .i32 = @bitCast(v) } },
            i64 => .{ .kind = .i64, .of = .{ .i64 = v } },
            u64 => .{ .kind = .i64, .of = .{ .i64 = @bitCast(v) } },
            [*]u8 => .{ .kind = .i32, .of = .{ .i32 = @bitCast(@as(u32, @intCast(v - memory.data()))) } },
            f32 => .{ .kind = .f32, .of = .{ .f32 = v } },
            f64 => .{ .kind = .f64, .of = .{ .f64 = v } },
            else => @compileError("Unsupported return type " ++ @typeName(@TypeOf(v))),
        };
    }
    ...
    pub fn call(self: *Func, comptime ResultType: type, args: anytype) CallError!ResultType {
        ...
        const args_len = args.len;
        var wasm_args: [args_len]Value = undefined; // Remove 'comptime' from here
        ...
        const final_args = ValVec{
            .size = args_len,
            .data = if (args_len == 0) undefined else &wasm_args, // Remove @as and @ptrCast
        };
        ...
        if (trap) |t| {
            log.err("code unexpectedly trapped: {s}", .{t.message().toSlice()});
            t.deinit();
            return CallError.Trap;
        }
    ...
    fn typeToValKind(comptime T: type) Valkind {
        return switch (T) {
            i32, u32 => .i32,
            i64, u64 => .i64,
            [*]u8 => .i32,
    ...
    extern "c" fn wasm_func_new_with_env(*Store, ?*anyopaque, *const Callback, *anyopaque, ?*const fn(*anyopaque) void) ?*Func;
...
pub const Instance = opaque {
    ...
    pub fn init(store: *Store, module: *Module, imports: anytype) !*Instance {
        std.debug.assert(@typeInfo(@TypeOf(imports)) == .@"struct");

        const imports_named = @typeInfo(@TypeOf(imports)).@"struct".fields;
        ...
        var externs = ExternVec.initWithCapacity(imports_named.len);
        defer externs.deinit();

        for (import_types.data[0..import_types.size], 0..) |imp, i| {
            const name = imp.?.wasm_importtype_name().?.toSlice();

            const match: *Func = blk: inline for (imports_named)  |in| {
                if (std.mem.eql(u8, in.name, name)) {
                    break :blk @field(imports, in.name);
                }
            } else return error.MissingImport;

            externs.data[i] = match.asExtern();
        }
...
pub const ImportType = opaque {
    extern "c" fn wasm_importtype_module(*ImportType) ?*ByteVec;
...
pub const Callback = fn (*anyopaque, *const ValVec, *ValVec) callconv(c_callconv) ?*Trap;
...
pub const ValtypeVec = extern struct {
    ...
    pub fn initWithCapacity(size: usize) ValtypeVec {
        var bytes: ValtypeVec = undefined;
        wasm_valtype_vec_new_uninitialized(&bytes, size);
        return bytes;
    }

    pub fn deinit(self: *ValtypeVec) void {
        self.wasm_valtype_vec_delete();
    }
