...
pub fn onOpen() void {
	...
	textComponent = TextInput.init(.{0, 0}, width, 32, if(settings.playerName.len == 0) "quanturmdoelvloper" else settings.playerName, .{.callback = .initFromCode(&apply)}, .{});
	list.add(textComponent);
	list.add(Button.initText(.{0, 0}, 100, "Apply", .{.callback = .initFromCode(&apply)}));
	list.finish(.center);
