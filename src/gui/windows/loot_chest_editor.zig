...
pub fn openFromLootChestData(_pos: main.vec.Vec3i, _oldText: []const u8) void {
	pos = _pos;
	main.globalAllocator.free(oldText);
...
fn apply() void {
	...
	main.block_entity.BlockEntityTypes.LootChest.updateTextFromClient(pos, textComponent.currentString.items);
...
pub fn onOpen() void {
	...
	textComponent = TextInput.init(.{0, 0}, width, 16 + 8, oldText, .{.onNewline = .init(apply)});
