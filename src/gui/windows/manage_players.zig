	...
	const list = VerticalList.init(.{padding, 16 + padding}, 300, 16);
	{
		...
		for(main.server.connectionManager.connections.items, 0..) |connection, i| {
			...
			if(connection.user.?.name.len != 0) {
				...
				row.add(Button.initText(.{0, 0}, 100, "Kick", .{.callback = .initFromCode(@ptrCast(&kick)), .arg = @intFromPtr(connection)}));
			} else {
				...
				row.add(Button.initText(.{0, 0}, 100, "Cancel", .{.callback = .initFromCode(@ptrCast(&kick)), .arg = @intFromPtr(connection)}));
			}
			list.add(row);
		}
	}
