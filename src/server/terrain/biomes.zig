...
const Stripe = struct { // MARK: Stripe
	...
	pub fn init(parameters: ZonElement) Stripe {
		...
		if (parameters.ZonObject.get("distance")) |dist| {
		...
		var minOffset: f64 = 0;
		var maxOffset: f64 = 0;
		if (parameters.ZonObject.get("offset")) |off| {
		...
		var minWidth: f64 = 0;
		var maxWidth: f64 = 0;
		if (parameters.ZonObject.get("width")) |width| {
...
pub const Biome = struct { // MARK: Biome
	...
	pub fn init(self: *Biome, id: []const u8, paletteId: u32, zon: ZonElement) void {
		self.* = Biome {
			...
			.chance = zon.get(f32, "chance", if(zon == .ZonNull) 0 else 1),
